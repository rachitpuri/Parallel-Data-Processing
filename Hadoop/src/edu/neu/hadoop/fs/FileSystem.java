 ...
 * 
 * @author Adib Alwani. Rachit Puri, Bhavin Vora
 */
public class FileSystem {
		...
		if (clusterMode) {
			try {
				...
				if (file.getFileName() != null) {
					...
					Process process = runtime.exec(Constants.s3ToLocal(
							bucketName + file.getPath(), file.getPath()));
					process.waitFor();
	...
	public FileStatus[] listStatus(Path path) {
		...
		FileStatus[] fileStatus = new FileStatus[listOfFiles.length];

		for (int i = 0; i < listOfFiles.length; i++) {
			fileStatus[i] = new FileStatus(new Path(listOfFiles[i].getName()));
		}
