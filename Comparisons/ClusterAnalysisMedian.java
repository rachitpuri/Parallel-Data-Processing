...
public class ClusterAnalysisMedian extends Configured implements Tool {
	...
	public static void main(String[] args) {
		...
		try {
			System.exit(ToolRunner.run(new ClusterAnalysisMedian(), args));
		} catch (Exception e) {
			
		}
		long end = System.currentTimeMillis();
		long timeTaken = end - start;
		File file = new File("s3n://adibalwani/test.txt");
        try {
        	output = new BufferedWriter(new FileWriter(file));
			output.write(String.valueOf(timeTaken));
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (output != null) {
		...
		/*AWSCredentials credentials = new BasicAWSCredentials(
				"AKIAIVBPHI5IOAFANURA", 
				"UuFH+IlHYtg8U+wwC/4c/6NLIjKsUgFDwvu23NOh");
		// create a client connection based on credentials
		AmazonS3 s3client = new AmazonS3Client(credentials);
		// upload file to folder and set it to public
		String fileName = "test.txt";
		String bucketName = "adibrachit";
